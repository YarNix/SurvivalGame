...
class AbstractEntity:
    ...
    """
    def __init__(self, width = 10, height = 10, x = 0, y = 0, speed = 200, collisions: list[pg.rect.FRect] = []):
...
class Player(pg.sprite.Sprite, _TmxSprite, AbstractEntity):
    def __init__(self, *groups, collisions: list[pg.rect.FRect]):
        ...
        AbstractEntity.__init__(self, 11 * self.scaled, 13 * self.scaled, -0.5, 4.5, collisions = collisions)
...
class Enemy(pg.sprite.Sprite, _TmxSprite, AbstractEntity):
    def __init__(self, *groups, collisions: list[pg.rect.FRect] = [], player: Player | None = None, pathfind: PathFinder | None = None, onkill: Callable[['Enemy'], None] | None = None):
        ...
        AbstractEntity.__init__(self, 16 * self.scaled, 20 * self.scaled, -0.5, -1, speed=100, collisions = collisions)
        self.health = 100
    ...
    def update(self, *, dt: float, **kwargs):
        if self.health > 0: # Skip movement if dying
            if self.pathfind is not None:
                self.direction.update(self.pathfind.get_direction(self.state_rect, self.player.state_rect))
            elif self.player:
                self.direction.update(pg.Vector2(self.player.state_rect.center) - self.state_rect.center)
            else:
        ...
        if self.player.attacking:
            if self.player.attack_mask is not None and not (self.missed or self.damaged):
                ...
                MASK_BG.blit(self.player.attack_mask.to_surface())
                MASK_BG.blit(hitbox.to_surface(), offset)
